...
1. prompt 明确要求用 RTL 实现的技术文章。
2. 其他技术类分析，包括芯片、FPGA、算法、AI、网络技术、芯片内部互联总线、硬件设计等内容的入门和深入研究。
3. 金融、投资、股票、市场分析等基础知识和深入研究。

这个 skill 的设计目标不是写百科词条，也不是生成松散的 AI 风格文章，而是帮助文章具备清晰结构、严谨事实、图文并茂、可读表达和可持续扩展的写作规则，避免整篇文章干巴巴地堆满文字。
...
只有当 prompt 明确要求 RTL 实现、RTL/SystemVerilog 编写或 RTL 导向的实现与验证内容时，才使用第一类规则；其他技术类分析默认使用第二类规则。
...
- 文章结尾使用 `----`
- 图片、表格、参考资料规则
- 公式必须使用 Markdown LaTeX，正文公式默认用 `$$...$$`
- 图文并茂，避免大段干巴巴的纯文字
- 禁止参考本地已有 `.md` 文章的行文格式或内容，除非用户明确指定某个 `.md` 文件作为编辑对象
- 排版克制和最终自检
...
第一类文章规则：prompt 明确要求用 RTL 实现的技术文章。

适合写：

- 明确要求用 RTL 实现某个模块或算法
- 明确要求编写 RTL/SystemVerilog
- 明确要求 RTL 导向的实现与验证内容
- 架构与接口设计
- 子模块划分
- SystemVerilog RTL
...
第二类文章规则：其他技术类分析，包括芯片、FPGA、算法、AI、网络技术、芯片内部互联总线、硬件设计等内容的入门和深入研究。
